...
func (p *Provider) HandleWebhook(
	...
	if err != nil {
		return nil, fmt.Errorf(
			"parse paypal webhook event: %w: %w",
			payments.ErrWebhookVerification, err,
		)
	}
...
func (p *Provider) CaptureOrder(
	...
	headers := http.Header{}
	headers.Set("PayPal-Request-Id", paypalCaptureRequestID(orderID))
	resp, err := p.doAuthedRequestWithHeaders(
		ctx, http.MethodPost, endpoint, token, nil, headers,
	)
	if err != nil {
...
func paypalCaptureRequestID(orderID string) string {
	return "virtuestack-capture-" + orderID
}
...
func (p *Provider) doAuthedRequest(
	...
	return p.doAuthedRequestWithHeaders(ctx, method, endpoint, token, body, nil)
}

func (p *Provider) doAuthedRequestWithHeaders(
	ctx context.Context,
