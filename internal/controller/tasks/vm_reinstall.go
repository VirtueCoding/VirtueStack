...
//  3. Transition to reinstalling
//  4. Reimage through Node Agent
//  5. Update database
func handleVMReinstall(ctx context.Context, task *models.Task, deps *HandlerDeps) error {
	...
	if done := checkReinstallIdempotency(ctx, deps, task, &payload, info, logger); done {
		return nil
	}
	...
	// Step 4: Update status and reimage VM through Node Agent
	if err := deps.TaskRepo.UpdateProgress(ctx, task.ID, 10, "Preparing VM reinstallation..."); err != nil {
		logger.Warn("failed to update task progress", "error", err)
	}
	...
	reinstallReq, err := buildReinstallRequest(ctx, deps, &payload, info, passwordHash, logger)
	if err != nil {
		return err
	}

	if err := deps.TaskRepo.UpdateProgress(ctx, task.ID, 40, "Reinstalling VM..."); err != nil {
		logger.Warn("failed to update task progress", "error", err)
	}
	if _, err := deps.NodeClient.ReinstallVM(ctx, info.nodeID, reinstallReq); err != nil {
		logger.Error("failed to reinstall VM via node agent", "error", err)
		return fmt.Errorf("reinstalling VM %s via node agent: %w", payload.VMID, err)
	}
