...
func (m *DHCPManager) GenerateDNSMasqConfigFull(cfg DHCPConfig) (string, error) {
	...
	// Validate MAC address to prevent newline/directive injection into the
	// dhcp-host= line. net.ParseMAC rejects values containing newlines or
	// other non-hex characters.
	if _, err := net.ParseMAC(cfg.MACAddress); err != nil {
		return "", fmt.Errorf("invalid MAC address %q for VM %s: %w", cfg.MACAddress, cfg.VMID, err)
	}
