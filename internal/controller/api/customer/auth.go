...
// LoginRequest contains the customer credentials submitted during login.
type LoginRequest struct {
	Email    string `json:"email" validate:"required,email,max=254"`
	Password string `json:"password" validate:"required,min=12,max=128"`
}

// Verify2FARequest contains the temporary token and TOTP code for customer 2FA.
type Verify2FARequest struct {
	TempToken string `json:"temp_token" validate:"required"`
	TOTPCode  string `json:"totp_code" validate:"required,len=6,numeric"`
}

// ChangePasswordRequest contains the current and replacement customer passwords.
...
// AuthResponse returns the token metadata for a customer authentication step.
...
func loginErrorResponse(err error) (int, string, string) {
	...
	if sharederrors.Is(err, sharederrors.ErrAccountLocked) || sharederrors.Is(err, sharederrors.ErrRateLimited) {
		return http.StatusTooManyRequests, "ACCOUNT_LOCKED", "Too many failed login attempts. Please try again later."
	}

	return http.StatusInternalServerError, "LOGIN_FAILED", "Internal server error"
}
...
// Login authenticates a customer with email and password.
func (h *CustomerHandler) Login(c *gin.Context) {
	...
	if err != nil {
		...
		status, code, message := loginErrorResponse(err)
...
// Verify2FA completes the customer 2FA flow and issues session tokens.
// @Tags Customer
...
// RefreshToken refreshes a customer access token using the refresh cookie.
// @Tags Customer
...
// Logout invalidates the current customer session and clears auth cookies.
// @Tags Customer
...
// ChangePassword updates the authenticated customer's password.
// @Tags Customer
