...
func TestBuildCheckoutParamsCopiesMetadataToPaymentIntent(t *testing.T) {
	tests := []struct {
		name string
		req  payments.PaymentRequest
	}{
		{
			name: "copies session metadata to payment intent metadata",
			req: payments.PaymentRequest{
				CustomerID:    "cust_1",
				AmountCents:   2500,
				Currency:      "usd",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			p := testProvider("whsec_test")
			params := p.buildCheckoutParams(tt.req)

			require.Equal(t, "cust_1", params.Metadata["customer_id"])
			require.Equal(t, "2500", params.Metadata["amount_cents"])
			require.Equal(t, "pay_1", params.Metadata["payment_id"])
			require.NotNil(t, params.PaymentIntentData)
			require.Equal(t, "cust_1", params.PaymentIntentData.Metadata["customer_id"])
			require.Equal(t, "2500", params.PaymentIntentData.Metadata["amount_cents"])
			require.Equal(t, "pay_1", params.PaymentIntentData.Metadata["payment_id"])
		})
	}
}
