...
func (p *Provider) executeVerify(
	...
	if resp.StatusCode != http.StatusOK {
		...
		case http.StatusBadRequest, http.StatusUnauthorized, http.StatusForbidden:
			return fmt.Errorf(
				"paypal verify webhook failed (status %d): %w: %s",
				resp.StatusCode, payments.ErrWebhookVerification,
				string(respBody),
			)
		}
	...
	if verifyResp.VerificationStatus != "SUCCESS" {
		return fmt.Errorf(
			"paypal webhook verification failed: %w: %s",
