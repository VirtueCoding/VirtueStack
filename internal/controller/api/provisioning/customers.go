...
func (h *ProvisioningHandler) CreateOrGetCustomer(c *gin.Context) {
	...
	if err == nil && existing != nil {
		if err := h.updateExternalClientID(c, existing, req.ExternalClientID); err != nil {
			h.respondExternalClientIDError(c, existing.ID, err)
			return
		}
...
func (h *ProvisioningHandler) updateExternalClientID(c *gin.Context, customer *models.Customer, externalClientID *int) error {
	...
	customer.ExternalClientID = externalClientID
	return nil
}

func (h *ProvisioningHandler) respondExternalClientIDError(c *gin.Context, customerID string, err error) {
	if sharederrors.Is(err, sharederrors.ErrConflict) {
		h.logger.Warn("external_client_id conflict on existing customer",
			"customer_id", customerID,
			"correlation_id", middleware.GetCorrelationID(c))
		middleware.RespondWithError(c, http.StatusConflict, "EXTERNAL_CLIENT_CONFLICT", "Customer belongs to a different external client")
		return
	}
