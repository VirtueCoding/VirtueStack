...
var templateImageSanitizer = regexp.MustCompile(`[^a-z0-9]+`)
...
func (h *AdminHandler) CreateTemplate(c *gin.Context) {
	...
	template := templateFromCreateRequest(req)
...
func templateFromCreateRequest(req models.TemplateCreateRequest) *models.Template {
	storageBackend := req.StorageBackend
	if storageBackend == "" {
		storageBackend = services.StorageBackendCeph
	}
	...
	rbdImage := strings.TrimSpace(req.RBDImage)
	if rbdImage == "" && storageBackend == services.StorageBackendCeph {
		rbdImage = defaultTemplateImageName(req.Name)
	}

	rbdSnapshot := strings.TrimSpace(req.RBDSnapshot)
	if rbdSnapshot == "" && storageBackend == services.StorageBackendCeph {
		rbdSnapshot = "v1"
	}

	minDiskGB := req.MinDiskGB
	if minDiskGB < 1 {
		minDiskGB = services.DefaultMinDiskGB
	}
	...
	var filePath *string
	if trimmed := strings.TrimSpace(req.FilePath); trimmed != "" {
		filePath = &trimmed
	}
	...
	return &models.Template{
		...
		MinDiskGB:         minDiskGB,
		SupportsCloudInit: supportsCloudInit,
		IsActive:          isActive,
...
func defaultTemplateImageName(name string) string {
	sanitized := strings.ToLower(strings.TrimSpace(name))
	sanitized = templateImageSanitizer.ReplaceAllString(sanitized, "-")
