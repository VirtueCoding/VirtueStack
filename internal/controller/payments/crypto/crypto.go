...
// ErrWebhookVerification marks crypto webhooks rejected before trusted processing.
var ErrWebhookVerification = errors.New("crypto webhook verification failed")

// ErrWebhookValidation marks verified crypto webhooks with invalid provider data.
var ErrWebhookValidation = errors.New("crypto webhook validation failed")

// IsWebhookClientError reports whether err should be returned as HTTP 400.
func IsWebhookClientError(err error) bool {
	return errors.Is(err, ErrWebhookVerification) ||
		errors.Is(err, ErrWebhookValidation)
}
