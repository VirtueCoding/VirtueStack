...
```bash
cd tests/e2e
pnpm install --frozen-lockfile --ignore-scripts  # Install E2E deps; install Playwright browsers separately
...
- Do not mix them — use `npm ci` for WebUIs and `pnpm install --frozen-lockfile --ignore-scripts` for E2E tests (then install Playwright browsers separately).
