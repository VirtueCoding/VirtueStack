...
class Virtuestack extends \Module
{
    ...
    public function addService(
        ...
        $clientId = (int) ($vars['client_id'] ?? 0);
        $serviceId = (int) ($vars['service_id'] ?? 0);
        if ($clientId <= 0) {
            $this->Input->setErrors([
                'client_id' => [
                    'invalid' => Language::_(
                        'Virtuestack.error.client_id_required',
                        true
                    ),
                ],
            ]);
        ...
        if ($serviceId <= 0) {
            $this->Input->setErrors([
                'service_id' => [
                    'invalid' => Language::_(
                        'Virtuestack.error.service_id_required',
                        true
                    ),
                ],
            ]);
        ...
        try {
            $existing = $api->getVMByServiceId($serviceId, $clientId);
    ...
    public function cancelService(
        ...
        try {
            $api = $this->getApi($moduleRow);
            $api->deleteVM($vmId, (int) ($service->id ?? 0), (int) ($service->client_id ?? 0));
        } catch (Exception $e) {
            $this->Input->setErrors([
                'vm' => [
                    'cancel' => sprintf(
    ...
    public function suspendService(
        ...
        try {
            $api = $this->getApi($moduleRow);
            $api->suspendVM($vmId, (int) ($service->id ?? 0), (int) ($service->client_id ?? 0));
        } catch (Exception $e) {
            $this->Input->setErrors([
                'vm' => [
                    'suspend' => sprintf(
    ...
    public function unsuspendService(
        ...
        try {
            $api = $this->getApi($moduleRow);
            $api->unsuspendVM($vmId, (int) ($service->id ?? 0), (int) ($service->client_id ?? 0));
        } catch (Exception $e) {
            $this->Input->setErrors([
                'vm' => [
                    'unsuspend' => sprintf(
    ...
    public function editService(
        ...
        try {
            $api = $this->getApi($moduleRow);
            $api->resizeVM($vmId, (int) ($service->id ?? 0), (int) ($service->client_id ?? 0), $newPlanId);
        } catch (Exception $e) {
            $this->Input->setErrors([
                'vm' => [
                    'resize' => sprintf(
    ...
    public function tabAdminService(
        ...
        if (!empty($post['action']) && !empty($vmId) && $moduleRow) {
            try {
                ...
                switch ($post['action']) {
                    case 'start':
                        $api->powerOperation($vmId, (int) ($service->id ?? 0), (int) ($service->client_id ?? 0), 'start');
                        break;
                    case 'stop':
                        $api->powerOperation($vmId, (int) ($service->id ?? 0), (int) ($service->client_id ?? 0), 'stop');
                        break;
                    case 'restart':
                        $api->powerOperation($vmId, (int) ($service->id ?? 0), (int) ($service->client_id ?? 0), 'restart');
                        break;
                    case 'force_stop':
                        $api->powerOperation($vmId, (int) ($service->id ?? 0), (int) ($service->client_id ?? 0), 'stop');
                        break;
        ...
        if (!empty($vmId) && $moduleRow) {
            try {
                $api = $this->getApi($moduleRow);
                $vmInfo = $api->getVMInfo($vmId, (int) ($service->id ?? 0), (int) ($service->client_id ?? 0));
                $statusData = $api->getVMStatus($vmId, (int) ($service->id ?? 0), (int) ($service->client_id ?? 0));
            ...
            if (!empty($post['action'])) {
                try {
                    ...
                    if (in_array($action, ['start', 'stop', 'restart'], true)) {
                        $api->powerOperation($vmId, (int) ($service->id ?? 0), (int) ($service->client_id ?? 0), $action);
                    } elseif ($action === 'reset_password') {
                        $api->resetPassword($vmId, (int) ($service->id ?? 0), (int) ($service->client_id ?? 0));
                    }
                } catch (Exception $e) {
                    $this->view->set('error', $e->getMessage());
                }
