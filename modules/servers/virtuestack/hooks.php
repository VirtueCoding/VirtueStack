...
function pollProvisioningTask(array $service): void
{
    ...
    if ($task['status'] === 'completed') {
        handleProvisioningComplete($serviceId, (int) ($service['client_id'] ?? 0), $task, $client);
 ...
 */
function handleProvisioningComplete(int $serviceId, int $clientId, array $task, ApiClient $client): void
{
    $result = $task['result'] ?? [];
    ...
    // Get VM details
    try {
        $vmInfo = $client->getVMInfo($vmId, $serviceId, $clientId);
...
function syncVMStatuses(): void
{
    ...
    foreach ($services as $service) {
        try {
            ...
            $status = $client->getVMStatus($vmId, (int) $service['service_id'], (int) ($service['client_id'] ?? 0));
...
function getActiveVirtueStackServices(): array
{
    try {
        return Capsule::table('tblhosting')
            ...
            ->where('tblproducts.servertype', 'virtuestack')
            ->where('tblhosting.domainstatus', 'Active')
            ->select('tblhosting.id as service_id', 'tblhosting.userid as client_id')
            ->get()
            ->map(function ($row) {
                return (array) $row;
            })
