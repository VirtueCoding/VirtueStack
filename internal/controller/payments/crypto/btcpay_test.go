...
func TestBTCPay_HandleWebhook_SettledMissingInvoiceIDIsValidationError(t *testing.T) {
	var calls int32
	p := newTestBTCPayProvider(t, http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
	...
	payload := `{"deliveryId":"d3","type":"InvoiceSettled","storeId":"test-store"}`
	sig := "sha256=" + computeHMACSHA256("test-webhook-secret", []byte(payload))

	headers := http.Header{}
	headers.Set("BTCPay-Sig", sig)

	_, err := p.HandleWebhook(context.Background(), headers, []byte(payload))
	require.Error(t, err)
	assert.ErrorIs(t, err, ErrWebhookValidation)
	assert.Zero(t, atomic.LoadInt32(&calls))
}

func TestBTCPay_HandleWebhook_StoreMismatchIsValidationError(t *testing.T) {
	tests := []struct {
		name    string
		storeID string
	}{
		{name: "missing store", storeID: ""},
		{name: "wrong store", storeID: "other-store"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			payload := fmt.Sprintf(
				`{"deliveryId":"d4","type":"InvoiceSettled","invoiceId":"INV-SETTLED","storeId":%q}`,
				tt.storeID,
			)
			sig := "sha256=" + computeHMACSHA256("test-webhook-secret", []byte(payload))
