...
func TestNOWPayments_CreatePaymentSession_Success(t *testing.T) {
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal(t, "acct-1:pay-1", req.OrderID)
...
func TestNOWPayments_HandleWebhook_Finished(t *testing.T) {
	...
	assert.Equal(t, "pay-abc-123", result.InvoiceID)
	assert.Equal(t, "12345", result.ExternalPaymentID)
...
func TestNOWPayments_HandleWebhook_FinishedMissingPaymentIDIsValidationError(t *testing.T) {
	p, err := NewNOWPaymentsProvider(NOWPaymentsConfig{
		APIKey:    "test-api-key",
		IPNSecret: "test-ipn-secret",
		Logger:    testLogger(),
