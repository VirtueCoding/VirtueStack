...
function convertMemoryMBToGB(memoryMB: number): number {
  return Number((memoryMB / 1024).toFixed(1));
}
...
function normalizeNode(raw: RawNode): Node {
  return {
    ...
    status: raw.status,
    location: raw.location_id || "Unassigned",
    vm_count: 0,
    cpu_total: raw.total_vcpu,
    cpu_allocated: raw.allocated_vcpu,
    memory_total_gb: convertMemoryMBToGB(raw.total_memory_mb),
    memory_allocated_gb: convertMemoryMBToGB(raw.allocated_memory_mb),
  };
}
...
export const adminNodesApi = {
  async getNodes(params: { per_page?: number; cursor?: string } = {}): Promise<PaginatedResponse<Node>> {
    ...
    const response = await apiPaginatedGet<PaginatedResponse<RawNode>>(`/admin/nodes${query ? `?${query}` : ""}`);
    return {
      data: (response.data || []).map(normalizeNode),
      meta: response.meta,
    };
  },
...
export const adminCustomersApi = {
  async getCustomers(params: { per_page?: number; cursor?: string; status?: string; search?: string } = {}): Promise<PaginatedResponse<Customer>> {
    ...
    const query = searchParams.toString();
    return apiPaginatedGet<PaginatedResponse<Customer>>(`/admin/customers${query ? `?${query}` : ""}`);
  },
  ...
  async getCustomerAuditLogs(id: string, perPage = 20, cursor?: string): Promise<PaginatedResponse<AuditLog>> {
    ...
    return apiPaginatedGet<PaginatedResponse<AuditLog>>(`/admin/customers/${id}/audit-logs?${searchParams.toString()}`);
  },
};
...
export const adminPlansApi = {
  async getPlans(): Promise<Plan[]> {
    const plans = await apiClient.get<RawPlan[]>("/admin/plans");
  ...
  async createPlan(plan: CreatePlanRequest): Promise<Plan> {
    const created = await apiClient.post<RawPlan>("/admin/plans", plan);
    return normalizePlan(created);
  },

  async updatePlan(id: string, plan: UpdatePlanRequest): Promise<Plan> {
    const updated = await apiClient.put<RawPlan>(`/admin/plans/${id}`, plan);
...
export const adminAuditLogsApi = {
  async getAuditLogs(perPage = 20, search?: string, cursor?: string): Promise<PaginatedResponse<AuditLog>> {
    ...
    return apiPaginatedGet<PaginatedResponse<AuditLog>>(`/admin/audit-logs?${searchParams.toString()}`);
  },
};
...
export const adminVMsApi = {
  async getVMs(params: { per_page?: number; cursor?: string } = {}): Promise<PaginatedResponse<VM>> {
    ...
    if (params.cursor) searchParams.set("cursor", params.cursor);
    const query = searchParams.toString();
    return apiPaginatedGet<PaginatedResponse<VM>>(`/admin/vms${query ? `?${query}` : ""}`);
  },
...
type RawTemplate = Omit<Template, "status"> & {
  status?: string;
};
...
export const adminTemplatesApi = {
  async getTemplates(): Promise<Template[]> {
    const templates = await apiClient.get<RawTemplate[]>("/admin/templates");
    return (templates || []).map(normalizeTemplate);
  },

  async getTemplate(id: string): Promise<Template> {
    const template = await apiClient.get<RawTemplate>(`/admin/templates/${id}`);
  ...
  async createTemplate(data: {
    ...
    const created = await apiClient.post<RawTemplate>("/admin/templates", data);
    return normalizeTemplate(created);
  },

  async updateTemplate(id: string, data: UpdateTemplateRequest): Promise<Template> {
    const updated = await apiClient.put<RawTemplate>(`/admin/templates/${id}`, data);
...
export const adminBackupsApi = {
  async getBackups(params: AdminBackupListParams = {}): Promise<PaginatedResponse<AdminBackup>> {
    ...
    return apiPaginatedGet<PaginatedResponse<AdminBackup>>(`/admin/backups?${searchParams.toString()}`);
  },
...
export const adminBackupSchedulesApi = {
  async getSchedules(params: { per_page?: number; cursor?: string; active?: boolean } = {}): Promise<PaginatedResponse<AdminBackupSchedule>> {
    ...
    return apiPaginatedGet<PaginatedResponse<AdminBackupSchedule>>(`/admin/admin-backup-schedules?${searchParams.toString()}`);
  },
...
export const adminIPSetsApi = {
  ...
  async getAvailableIPs(id: string, perPage = 50, cursor?: string): Promise<PaginatedResponse<{ id: string; address: string }>> {
    ...
    searchParams.set("per_page", String(perPage));
    if (cursor) searchParams.set("cursor", cursor);
    return apiPaginatedGet<PaginatedResponse<{ id: string; address: string }>>(
...
export const adminFailoverRequestsApi = {
  async getFailoverRequests(params: {
    ...
    return apiPaginatedGet<PaginatedResponse<FailoverRequest>>(endpoint);
  },
...
export const adminBillingApi = {
  async getTransactions(
    ...
    return apiPaginatedGet<PaginatedResponse<BillingTransaction>>(
  ...
  async getPayments(
    ...
    const query = queryParams.toString() ? `?${queryParams.toString()}` : "";
    return apiPaginatedGet<PaginatedResponse<BillingPayment>>(
...
export const adminInvoicesApi = {
  async list(
    ...
    return apiPaginatedGet<PaginatedResponse<Invoice>>(
