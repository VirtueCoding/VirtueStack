...
func TestChangePassword_InvalidRequestBody(t *testing.T) {
	...
	req := httptest.NewRequestWithContext(context.Background(), http.MethodPut, "/password", bytes.NewBufferString("invalid json"))
...
func TestChangePassword_PasswordTooShort(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequestWithContext(context.Background(), http.MethodPut, "/password", bytes.NewBuffer(jsonBody))
			req.Header.Set("Content-Type", "application/json")
			w := httptest.NewRecorder()
...
func TestLoginErrorResponse(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			err:         sharederrors.ErrUnauthorized,
		...
		{
			name:        "account locked",
			err:         sharederrors.ErrAccountLocked,
			wantStatus:  http.StatusTooManyRequests,
			wantCode:    "ACCOUNT_LOCKED",
			wantMessage: "Too many failed login attempts. Please try again later.",
		},
		{
			name:        "unexpected error",
			err:         stderrors.New("boom"),
			wantStatus:  http.StatusInternalServerError,
			wantCode:    "LOGIN_FAILED",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			status, code, message := loginErrorResponse(tt.err)
...
func TestChangePassword_JSONBinding(t *testing.T) {
	...
	t.Run("missing current_password field", func(t *testing.T) {
		...
		req := httptest.NewRequestWithContext(context.Background(), http.MethodPut, "/password", bytes.NewBufferString(body))
		req.Header.Set("Content-Type", "application/json")
		w := httptest.NewRecorder()
...
func TestChangePassword_ErrorCodes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := httptest.NewRequestWithContext(context.Background(), http.MethodPut, "/password", bytes.NewBufferString(tt.body))
			req.Header.Set("Content-Type", "application/json")
			w := httptest.NewRecorder()
