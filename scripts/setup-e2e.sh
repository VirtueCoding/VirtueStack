...
start_docker_services() {
    ...
    # Refuse to clobber an existing .env (which may be a real/prod environment)
    # unless the caller explicitly opts in via E2E_OVERWRITE_ENV=1.
    if [ -f .env.test ]; then
        if [ -f .env ] && [ "${E2E_OVERWRITE_ENV:-0}" != "1" ]; then
            log_error ".env already exists; refusing to overwrite. Set E2E_OVERWRITE_ENV=1 to replace it with .env.test."
            exit 1
        fi
        cp .env.test .env
    fi
