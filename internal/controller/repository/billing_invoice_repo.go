...
const invoicePeriodUniqueConstraint = "idx_billing_invoices_customer_period_unique"

// ErrDuplicateInvoicePeriod reports an invoice that already exists for the customer period.
var ErrDuplicateInvoicePeriod = errors.New("duplicate invoice period")
...
// NextInvoiceNumber atomically increments the invoice counter and returns a
// formatted invoice number like "INV-000001". Use CreateWithNextNumber for new
// invoices so allocation and insert share one transaction.
func (r *BillingInvoiceRepository) NextInvoiceNumber(ctx context.Context) (string, error) {
	return nextInvoiceNumber(ctx, r.db)
}

func nextInvoiceNumber(ctx context.Context, db invoiceQueryer) (string, error) {
	var nextNum int
	err := db.QueryRow(ctx,
...
func (r *BillingInvoiceRepository) Create(ctx context.Context, inv *models.BillingInvoice) error {
	if err := validateInvoiceTotals(inv); err != nil {
		return err
	}
	return insertInvoice(ctx, r.db, inv)
}

// CreateWithNextNumber allocates the invoice number and inserts the invoice in one transaction.
func (r *BillingInvoiceRepository) CreateWithNextNumber(
	ctx context.Context, inv *models.BillingInvoice,
) error {
	if err := validateInvoiceTotals(inv); err != nil {
		return err
	}
	...
	invoiceNumber, err := nextInvoiceNumber(ctx, tx)
	if err != nil {
		return err
	}
...
func insertInvoice(ctx context.Context, db invoiceExecer, inv *models.BillingInvoice) error {
	...
	if err != nil {
		if isInvoiceUniqueViolation(err) {
			return invoiceUniqueConflictError(err)
		}
...
func validateInvoiceTotals(inv *models.BillingInvoice) error {
	var lineItemTotal int64
	for _, item := range inv.LineItems {
		lineItemTotal += item.Amount
	}
	if inv.Subtotal != lineItemTotal {
		return fmt.Errorf("invoice subtotal does not match line items: %w",
			sharederrors.ErrValidation)
	}
	if inv.Total != inv.Subtotal+inv.TaxAmount {
		return fmt.Errorf("invoice total does not match subtotal plus tax: %w",
			sharederrors.ErrValidation)
	}
	return nil
}
...
func invoiceConflictConstraint(err error) string {
	var pgErr *pgconn.PgError
...
func invoiceUniqueConflictError(err error) error {
	constraint := invoiceConflictConstraint(err)
	if constraint == invoicePeriodUniqueConstraint {
		return fmt.Errorf("create invoice duplicate period: %w",
			errors.Join(ErrDuplicateInvoicePeriod, sharederrors.ErrConflict))
	}
	return fmt.Errorf("create invoice unique conflict %s: %w",
		constraint, sharederrors.ErrConflict)
}
