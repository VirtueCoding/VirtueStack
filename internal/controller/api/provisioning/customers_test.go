...
func TestCreateOrGetCustomer_ValidationErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "zero whmcs_client_id",
			body:     `{"email":"test@example.com","name":"Test User","whmcs_client_id":0}`,
			wantHTTP: http.StatusBadRequest,
			errCode:  "VALIDATION_ERROR",
		},
		{
			name:     "negative whmcs_client_id",
			body:     `{"email":"test@example.com","name":"Test User","whmcs_client_id":-1}`,
			wantHTTP: http.StatusBadRequest,
			errCode:  "VALIDATION_ERROR",
		},
	}
...
func TestCreateCustomerRequest_ValidFields(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "with whmcs_client_id",
			body:  `{"email":"test@example.com","name":"Test User","whmcs_client_id":42}`,
			email: "test@example.com",
			cname: "Test User",
		},
	}
