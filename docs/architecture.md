...
<iframe
  src="https://panel.{domain}/api/v1/customer/auth/sso-exchange?token={$opaque_token}"
  style="width: 100%; height: 80vh; border: none; border-radius: 8px;"
  allow="clipboard-read; clipboard-write"
></iframe>
```

The `opaque_token` is a short-lived single-use token minted by the Provisioning API and stored server-side. On first use the controller consumes the token, creates the normal customer session cookies, and redirects the browser to the clean `/vms/{id}` page without leaving a bearer token in the URL.
