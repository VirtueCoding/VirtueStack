...
func (h *grpcHandler) TransferDisk(req *nodeagentpb.TransferDiskRequest, stream nodeagentpb.NodeAgentService_TransferDiskServer) error {
	...
	if h.server.storageType == storage.StorageTypeQCOW && snapshotName != "" {
		// NA-3: snapshotName flows into `qemu-img -l <name>`. qemu-img parses
		// the value as a comma-separated key=value list and any leading dash
		// would be treated as a flag. Validate before constructing argv.
		if err := validateQCOWSnapshotName(snapshotName); err != nil {
...
func (h *grpcHandler) transferLVMDisk(ctx context.Context, req *nodeagentpb.TransferDiskRequest, stream nodeagentpb.NodeAgentService_TransferDiskServer, logger *slog.Logger) error {
	...
	// NA-2: validate VG and snapshot identifiers before concatenating them into
	// `/dev/<vg>/<snap>`; otherwise a malicious controller could smuggle option
	// flags or path components into blockdev/dd argv.
	if snapshotName != "" {
		if err := validateTransferLVMArgs(req.GetSourceLvmVolumeGroup(), snapshotName); err != nil {
	...
	// NA-2: previously this Wait()'s error was silently discarded, hiding a
	// failed dd (e.g. block-device read error) as a successful transfer. The
	// deferred Wait() reaps the child on early returns; the explicit Wait()
	// after the streaming loop surfaces a non-zero exit so the caller does not
	// see partial data as success.
	ddWaited := false
	defer func() {
		if !ddWaited {
			_ = ddCmd.Wait()
	...
	// NA-2: explicitly wait for dd and surface any non-zero exit. Without this,
	// dd failures (e.g. read errors on the source LV) appeared as successful
	// transfers because the deferred Wait()'s error was discarded.
	ddWaited = true
	if err := ddCmd.Wait(); err != nil {
		return status.Errorf(codes.Internal, "dd exited with error: %v", err)
	}
