...
// LoginRequest contains the admin credentials submitted during login.
type LoginRequest struct {
	Email    string `json:"email" validate:"required,email,max=254"`
	Password string `json:"password" validate:"required,min=12,max=128"`
}

// Verify2FARequest contains the temporary token and TOTP code for admin 2FA.
type Verify2FARequest struct {
	TempToken string `json:"temp_token" validate:"required"`
	TOTPCode  string `json:"totp_code" validate:"required,len=6,numeric"`
}

// AuthResponse returns the token metadata for an admin authentication step.
...
// Verify2FA completes the admin 2FA flow and issues session tokens.
// @Tags Admin
...
// RefreshToken refreshes an admin access token using the refresh cookie.
// @Tags Admin
...
// Logout invalidates the current admin session and clears auth cookies.
// @Tags Admin
