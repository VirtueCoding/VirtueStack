...
VirtueStack is a KVM/QEMU VM management platform for VPS hosting providers. Stack: Go backend (Controller + Node Agent), Next.js frontends (admin/customer portals), PostgreSQL database, NATS JetStream task queue, and multi-provider billing (WHMCS, Blesta, or native credit-based).
...
### PHP Billing Modules
```bash
# Syntax check all PHP billing modules (WHMCS + Blesta)
...
```
80 sequential migrations (000001–000080) in `migrations/`. Each migration has `.up.sql` and `.down.sql` files.
...
cd tests/e2e && pnpm install --frozen-lockfile --ignore-scripts && pnpm test
...
| `internal/controller/billing` | `registry_test.go`, `provider_test.go` | Billing provider abstraction |
| `internal/controller/payments` | `registry_test.go`, `provider_test.go` | Payment gateway abstraction |
...
- `docs/billing-blesta.md` — Blesta module installation and configuration guide
...
- `internal/controller/billing/` — Billing provider abstraction (WHMCS, Blesta, Native)
- `internal/controller/payments/` — Payment gateway abstraction (Stripe, PayPal, crypto)
