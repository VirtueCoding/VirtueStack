...
type provisioningOwnershipAssertion struct {
	ExternalServiceID int
	ExternalClientID  *int
	CustomerID        string
}

func parseProvisioningOwnership(c *gin.Context) (provisioningOwnershipAssertion, error) {
	serviceID, err := strconv.Atoi(c.Query("external_service_id"))
	if err != nil || serviceID <= 0 {
		return provisioningOwnershipAssertion{}, vmValidationError{
			status:  http.StatusNotFound,
			errCode: "VM_NOT_FOUND",
			errMsg:  "VM not found",
		}
	}
	assertion := provisioningOwnershipAssertion{
		ExternalServiceID: serviceID,
		CustomerID:        c.Query("customer_id"),
	}
	if raw := c.Query("external_client_id"); raw != "" {
		externalClientID, err := strconv.Atoi(raw)
		if err != nil || externalClientID <= 0 {
			return provisioningOwnershipAssertion{}, vmValidationError{
				status:  http.StatusBadRequest,
				errCode: "INVALID_EXTERNAL_CLIENT_ID",
				errMsg:  "external_client_id must be a positive integer",
			}
		}
		assertion.ExternalClientID = &externalClientID
	}
	if assertion.CustomerID == "" && assertion.ExternalClientID == nil {
		return provisioningOwnershipAssertion{}, vmValidationError{
			status:  http.StatusBadRequest,
...
func (h *ProvisioningHandler) getValidOwnedVM(c *gin.Context, vmID string) (*models.VM, error) {
	if _, err := validateVMID(vmID); err != nil {
		return nil, err
	}
	assertion, err := parseProvisioningOwnership(c)
	if err != nil {
		return nil, err
	}
	...
	if !matchesProvisioningOwnership(vm, assertion) {
		return nil, vmNotFoundForProvisioning()
	}
	if err := h.validateOptionalCustomerAssertion(c.Request.Context(), vm, assertion); err != nil {
		return nil, err
	}
	return vm, nil
}

func (h *ProvisioningHandler) getOwnedVMIncludingDeleted(c *gin.Context, vmID string) (*models.VM, error) {
	assertion, err := parseProvisioningOwnership(c)
	if err != nil {
		return nil, err
	}
	if _, err := uuid.Parse(vmID); err != nil {
		return nil, vmValidationError{status: http.StatusBadRequest, errCode: "INVALID_VM_ID", errMsg: "VM ID must be a valid UUID"}
	}
	vm, err := h.vmRepo.GetByIDIncludingDeleted(c.Request.Context(), vmID)
	if err != nil {
		if sharederrors.Is(err, sharederrors.ErrNotFound) {
			return nil, vmNotFoundForProvisioning()
		}
		h.logger.Error("failed to get VM", "vm_id", vmID, "error", err)
		return nil, vmValidationError{status: http.StatusInternalServerError, errCode: "VM_LOOKUP_FAILED", errMsg: "Internal server error"}
	}
	if !matchesProvisioningOwnership(vm, assertion) {
		return nil, vmNotFoundForProvisioning()
	}
	return vm, h.validateOptionalCustomerAssertion(c.Request.Context(), vm, assertion)
}

func (h *ProvisioningHandler) getValidOwnedVMWithChecks(c *gin.Context, vmID string, opts checkVMOpts) (*models.VM, error) {
	...
	if opts.NotSuspended && vm.Status == models.VMStatusSuspended {
		...
		return nil, vmValidationError{status: http.StatusBadRequest, errCode: "VM_SUSPENDED", errMsg: errMsg}
	}
	return vm, nil
}

func matchesProvisioningOwnership(vm *models.VM, assertion provisioningOwnershipAssertion) bool {
	if vm.ExternalServiceID == nil || *vm.ExternalServiceID != assertion.ExternalServiceID {
		return false
	}
	if assertion.CustomerID != "" && assertion.CustomerID != vm.CustomerID {
		return false
	}
	return true
}

func (h *ProvisioningHandler) validateOptionalCustomerAssertion(ctx context.Context, vm *models.VM, assertion provisioningOwnershipAssertion) error {
	if assertion.ExternalClientID == nil {
		return nil
	}
	if h.customerRepo == nil {
		return vmNotFoundForProvisioning()
	}
	customer, err := h.customerRepo.GetByID(ctx, vm.CustomerID)
	if err != nil {
		return vmNotFoundForProvisioning()
	}
	if customer.ExternalClientID == nil || *customer.ExternalClientID != *assertion.ExternalClientID {
		return vmNotFoundForProvisioning()
	}
	return nil
}

func vmNotFoundForProvisioning() error {
...
func validateResizeRequest(req ResizeRequest) error {
	if req.PlanID == "" && req.VCPU == nil && req.MemoryMB == nil && req.DiskGB == nil {
		return vmValidationError{
			status:  http.StatusBadRequest,
			errCode: "VALIDATION_ERROR",
			errMsg:  "At least one resize parameter (plan_id, vcpu, memory_mb, or disk_gb) must be provided",
		}
	}
	return nil
}
