...
Designed for integration with billing systems like WHMCS and Blesta. VM-ID endpoints require a positive `external_service_id` query parameter plus at least one owner assertion (`customer_id` or `external_client_id`). The service ID and owner assertion must both match the VM. The by-service lookup has the same owner-assertion requirement in addition to the path `service_id`. Ownership mismatches return `404` to avoid exposing cross-customer resources.
...
| `hostname` | string | Yes | RFC 1123, max 63 chars |
| `ssh_keys` | string[] | No | Max 10 keys, max 4096 chars each |
| `external_service_id` | integer | Yes | Positive external billing service identifier |
| `location_id` | string | No | Valid UUID |
...
Repeating a create request with the same `Idempotency-Key` returns the original non-empty task ID when available. Reusing an `external_service_id` without a durable create task returns `409 Conflict`; clients should retry with an idempotency key or reconcile the existing service binding.
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `external_service_id` | integer | Yes | External billing service ID bound to the VM |
| `customer_id` | string | Conditional | Expected VirtueStack customer UUID. Required when `external_client_id` is absent |
| `external_client_id` | integer | Conditional | Expected external billing client ID. Required when `customer_id` is absent |
...
- `400` — `OWNERSHIP_ASSERTION_REQUIRED`: Missing `customer_id` and `external_client_id`
- `404` — `VM_NOT_FOUND`: No VM with that service ID or ownership mismatch
...
| Field | Type | Required | Constraints |
|-------|------|----------|-------------|
| `vm_id` | string | Conditional | Required valid UUID for VM-scoped SSO |
| `external_service_id` | integer | Conditional | Required for VM-scoped SSO and forbidden for customer-only SSO; must be positive |
| `customer_id` | string | Conditional | Customer UUID for customer-only SSO, or ownership assertion for VM-scoped SSO |
| `external_client_id` | integer | Conditional | Alternative customer ownership assertion for VM-scoped SSO |

VM-scoped SSO requires all three assertions: `vm_id`, `external_service_id`, and either `customer_id` or `external_client_id`. All supplied assertions must match the VM and owner. Requests with `external_service_id` but no `vm_id` are rejected. Customer-only SSO is explicit: provide `customer_id` with optional `external_client_id`, and do not include `vm_id` or `external_service_id`; the redirect path will be `/vms`.
...
Requires the VM ownership query parameters described above.
...
**Error Responses:**
- Already-deleted VMs return `409 Conflict` with `VM_DELETE_CONFLICT` so retries do not report success without a durable delete task.
...
Resize VM resources to a validated plan (upgrade only for disk). Supports async disk resize. Requires the VM ownership query parameters described above.

**Request Body:**

| Field | Type | Required | Constraints |
|-------|------|----------|-------------|
| `plan_id` | string | Yes | Target plan UUID |

Raw resource fields are ignored; provisioning resize must use a plan.
...
Create a one-time SSO token for billing-system → Customer Portal seamless login. The token is opaque, stored hashed server-side, single-use, and expires after 5 minutes. Customer-only SSO is supported; VM-scoped SSO requires VM ID, external billing service ID, and an owner assertion.

**Request Body:**

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `customer_id` | string | Conditional | Customer-only SSO target, or ownership assertion for VM-scoped SSO |
| `vm_id` | string | Conditional | Required VM UUID for VM-scoped SSO |
| `external_service_id` | integer | Conditional | Required for VM-scoped SSO and forbidden for customer-only SSO; must be positive |
| `external_client_id` | integer | Conditional | Alternative ownership assertion for VM-scoped SSO |

VM-scoped SSO requires `vm_id`, `external_service_id`, and either `customer_id` or `external_client_id`; all assertions must match. For customer-only SSO, send `customer_id` with optional `external_client_id` and no VM selector or service ID; the redirect path is `/vms`.
...
| `external_service_id` | integer | External billing service reference |
