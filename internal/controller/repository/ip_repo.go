...
func scanIPSet(row pgx.Row) (models.IPSet, error) {
	...
	err := row.Scan(
		&ipSet.ID, &ipSet.Name, &ipSet.LocationID, &network,
		&gateway, &ipSet.VLANID, &ipSet.IPVersion, &ipSet.NodeIDs,
		&ipSet.CreatedAt,
	)
	if err != nil {
		return ipSet, err
	}
	ipSet.Network = network.String()
	ipSet.Gateway = gateway.String()
	return ipSet, err
}
...
func (r *IPRepository) ListIPSets(ctx context.Context, filter IPSetListFilter) ([]models.IPSet, bool, string, error) {
	...
	whereBuilder := baseBuilder.Select(ipSetSelectCols).From("ip_sets")
	...
	listBuilder = listBuilder.OrderBy("id DESC").Limit(cursorPageLimit(filter.PerPage))
...
func scanIPAddress(row pgx.Row) (models.IPAddress, error) {
	...
	err := row.Scan(
		&ip.ID, &ip.IPSetID, &address, &ip.IPVersion,
...
func (r *IPRepository) ListIPAddresses(ctx context.Context, filter IPAddressListFilter) ([]models.IPAddress, bool, string, error) {
	baseBuilder := sq.StatementBuilder.PlaceholderFormat(sq.Dollar)
	whereBuilder := baseBuilder.Select(ipAddressSelectCols).From("ip_addresses")
	...
	listBuilder = listBuilder.OrderBy("id DESC").Limit(uint64(perPage + 1))
	listQ, listArgs, err := listBuilder.ToSql()
	if err != nil {
