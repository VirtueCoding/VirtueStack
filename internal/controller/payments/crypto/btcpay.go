...
func (p *BTCPayProvider) HandleWebhook(
	...
	if err := verifyBTCPaySignature(p.webhookSecret, sig, body); err != nil {
		return nil, fmt.Errorf(
			"btcpay webhook signature invalid: %w: %w",
	...
	if payload.StoreID == "" || payload.StoreID != p.storeID {
		return nil, fmt.Errorf(
			"btcpay store mismatch: %w",
			ErrWebhookValidation,
		)
	}
	if payload.InvoiceID == "" {
		return nil, fmt.Errorf(
			"btcpay InvoiceSettled webhook missing invoiceId: %w",
			ErrWebhookValidation,
		)
	}
	...
	if accountID == "" {
		return nil, fmt.Errorf(
			"btcpay invoice %s missing account_id metadata: %w",
			payload.InvoiceID, ErrWebhookValidation,
		)
	}
