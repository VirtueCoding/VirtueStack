...
func (h *ProvisioningHandler) GetVMInfo(c *gin.Context) {
	...
	if _, err := h.getValidOwnedVM(c, vmID); err != nil {
		respondWithValidationError(c, err)
		return
	}
...
func (h *ProvisioningHandler) GetVMByExternalServiceID(c *gin.Context) {
	...
	assertion, err := h.parseByServiceOwnershipAssertion(c, serviceID)
	if err != nil {
		respondWithValidationError(c, err)
		return
	}
...
func (h *ProvisioningHandler) parseByServiceOwnershipAssertion(c *gin.Context, serviceID int) (provisioningOwnershipAssertion, error) {
