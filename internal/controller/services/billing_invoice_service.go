...
func (s *BillingInvoiceService) buildAndCreateInvoice(
	...
	if err := s.invoiceRepo.CreateWithNextNumber(ctx, invoice); err != nil {
		if isDuplicatePeriodInvoiceConflict(err) {
			s.logger.Info("invoice already exists for period after create conflict",
				"customer_id", customerID,
				"period_start", periodStart,
				"period_end", periodEnd)
			return nil, nil
	...
	s.logger.Info("invoice generated",
		"invoice_id", invoice.ID, "invoice_number", invoice.InvoiceNumber,
...
func isDuplicatePeriodInvoiceConflict(err error) bool {
	return errors.Is(err, repository.ErrDuplicateInvoicePeriod)
}
