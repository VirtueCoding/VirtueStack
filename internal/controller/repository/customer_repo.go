...
func (r *CustomerRepository) UpdateExternalClientID(ctx context.Context, id string, externalClientID int) error {
	const q = `UPDATE customers SET external_client_id = $1, updated_at = NOW()
		WHERE id = $2 AND status != 'deleted'
		AND (external_client_id IS NULL OR external_client_id = $1)`
	tag, err := r.db.Exec(ctx, q, externalClientID, id)
	if err != nil {
		return fmt.Errorf("updating customer %s external client ID: %w", id, err)
	}
	if tag.RowsAffected() == 0 {
		return fmt.Errorf("updating customer %s external client ID: %w", id, sharederrors.ErrConflict)
	}
	return nil
}
...
func (r *CustomerRepository) CreateSession(ctx context.Context, session *models.Session) error {
	return insertSession(ctx, r.db, session)
}

func insertSession(ctx context.Context, db DB, session *models.Session) error {
	const q = `
...
// RotateSession atomically consumes an old refresh session and creates its replacement.
func (r *CustomerRepository) RotateSession(ctx context.Context, oldSessionID string, newSession *models.Session) error {
	tx, err := r.db.Begin(ctx)
	if err != nil {
		return fmt.Errorf("beginning session rotation transaction: %w", err)
	}
	defer tx.Rollback(ctx) //nolint:errcheck

	tag, err := tx.Exec(ctx, `DELETE FROM sessions WHERE id = $1`, oldSessionID)
	if err != nil {
		return fmt.Errorf("deleting old session %s during rotation: %w", oldSessionID, err)
	}
	if tag.RowsAffected() != 1 {
		return fmt.Errorf("deleting old session %s during rotation: %w", oldSessionID, ErrNoRowsAffected)
	}
	if err := insertSession(ctx, tx, newSession); err != nil {
		return fmt.Errorf("creating replacement session during rotation: %w", err)
	}
	if err := tx.Commit(ctx); err != nil {
		return fmt.Errorf("committing session rotation: %w", err)
	}
	return nil
}
