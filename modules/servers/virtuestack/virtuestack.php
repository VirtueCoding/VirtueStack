...
function virtuestack_CreateAccount(array $params): string
{
    ...
    try {
        ...
        try {
            $existingVM = $client->getVMByServiceId($serviceId, $clientId);
...
function virtuestack_SuspendAccount(array $params): string
{
    ...
    try {
        ...
        $result = $client->suspendVM($vmId, $serviceId, $clientId);
...
function virtuestack_UnsuspendAccount(array $params): string
{
    ...
    try {
        ...
        $result = $client->unsuspendVM($vmId, $serviceId, $clientId);
...
function virtuestack_TerminateAccount(array $params): string
{
    ...
    try {
        ...
        $result = $client->deleteVM($vmId, $serviceId, $clientId);
...
function virtuestack_ChangePackage(array $params): string
{
    ...
    try {
        ...
        // Call resize API
        $result = $client->resizeVM($vmId, $serviceId, $clientId, $newPlanId);
...
function virtuestack_ChangePassword(array $params): string
{
    ...
    try {
        ...
        $result = $client->resetPassword($vmId, $serviceId, $clientId);
...
function virtuestack_ClientArea(array $params): array
{
    ...
    try {
        ...
        if ($provisioningStatus === 'pending') {
            ...
            return [
                ...
                'vars' => [
                    ...
                    'task_id'         => $taskId,
                    'service_id'      => $serviceId,
                    'provisioningUrl' => virtuestack_getProvisioningStatusUrl($serviceId),
                ],
            ];
        }
        ...
        if (!empty($vmId) && !empty($webuiUrl)) {
            try {
                $sso = $client->createSSOToken($serviceId, $clientId, $vmId);
...
function virtuestack_syncStatus(array $params): string
{
    ...
    try {
        ...
        $status = $client->getVMStatus($vmId, $serviceId, $clientId);
 ...
 * @return string
 */
function virtuestack_getProvisioningStatusUrl(int $serviceId): string
{
    return "clientarea.php?action=productdetails&id={$serviceId}";
}
...
function virtuestack_powerOperation(array $params, string $operation): string
{
    ...
    try {
        ...
        $result = $client->powerOperation($vmId, $serviceId, $clientId, $operation);
...
function virtuestack_UsageUpdate(array $params): array
{
    ...
    try {
        $client = virtuestack_getApiClient($params);
        $vm = virtuestack_syncServiceState($client, $serviceId, $clientId);
        if (empty($vm)) {
            return [];
        }
        ...
        if (!empty($vm['id'])) {
            try {
                $usage = $client->getVMUsage((string) $vm['id'], $serviceId, $clientId);
            } catch (\Exception $e) {
                VirtueStackHelper::log('UsageUpdate', 'Usage fetch failed, using defaults', $e->getMessage());
            }
        }
...
function virtuestack_AdminServicesTabFieldsSave(array $params): string
{
    ...
    try {
        $client = virtuestack_getApiClient($params);
        virtuestack_syncServiceState($client, $serviceId, $clientId);
        return '';
    } catch (\Exception $e) {
...
function virtuestack_syncServiceState(ApiClient $client, int $serviceId, int $clientId): array
{
    if ($serviceId <= 0 || $clientId <= 0) {
        return [];
    }

    $vm = $client->getVMByServiceId($serviceId, $clientId);
    if (empty($vm['id'])) {
        return [];
    }

    try {
        $detail = $client->getVMInfo((string) $vm['id'], $serviceId, $clientId);
        if (!empty($detail)) {
            $vm = $detail;
        }
    } catch (\Exception $e) {
