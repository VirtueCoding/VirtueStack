...
type idempotentTaskPublisher interface {
	PublishTaskWithIdempotencyKey(ctx context.Context, taskType string, payload map[string]any, idempotencyKey string) (string, error)
}
...
func (s *VMService) persistVMRecord(ctx context.Context, req *models.VMCreateRequest, deps vmCreateDeps, customerID string) (*models.VM, error) {
...
func (s *VMService) publishVMCreateTask(ctx context.Context, req *models.VMCreateRequest, vm *models.VM, deps vmCreateDeps, ipv4Address *models.IPAddress) (string, error) {
	...
	if req.IdempotencyKey != "" {
		if publisher, ok := s.taskPublisher.(idempotentTaskPublisher); ok {
			taskID, err = publisher.PublishTaskWithIdempotencyKey(ctx, models.TaskTypeVMCreate, taskPayload, req.IdempotencyKey)
		} else {
...
func (s *VMService) CreateVM(ctx context.Context, req *models.VMCreateRequest, customerID string) (*models.VM, string, error) {
	...
	if req.IdempotencyKey != "" {
		vm, taskID, found, err := s.resolveCreateIdempotency(ctx, req, customerID)
		if found || err != nil {
			return vm, taskID, err
		}
	}
	...
	if req.ExternalServiceID != nil && *req.ExternalServiceID > 0 {
		...
		if err == nil && existingVM != nil {
			if existingVM.CustomerID != customerID {
				return nil, "", fmt.Errorf("external service %d belongs to another customer: %w", *req.ExternalServiceID, sharederrors.ErrConflict)
			}
			...
			return nil, "", externalServiceWithoutTaskError(*req.ExternalServiceID)
		}
	}
	...
	vm, err := s.persistVMRecord(ctx, req, deps, customerID)
	if err != nil {
		if req.ExternalServiceID != nil && *req.ExternalServiceID > 0 && errors.Is(err, sharederrors.ErrConflict) {
			return s.resolveExternalServiceCreateConflict(ctx, *req.ExternalServiceID, customerID)
		}
		return nil, "", err
	}

	taskID, err := s.publishVMCreateTask(ctx, req, vm, deps, nil)
	if err != nil {
		return nil, "", err
	}
...
func (s *VMService) resolveCreateIdempotency(
	ctx context.Context,
	req *models.VMCreateRequest,
	customerID string,
) (*models.VM, string, bool, error) {
	if s.taskRepo == nil {
	...
	vm := s.vmFromTaskPayload(ctx, existingTask.Payload)
	if vm == nil {
		if req.ExternalServiceID != nil && *req.ExternalServiceID > 0 {
			return nil, "", false, nil
		}
		return nil, existingTask.ID, true, nil
	}
	if vm.CustomerID != customerID {
		return nil, "", true, fmt.Errorf("idempotency key belongs to another customer: %w", sharederrors.ErrConflict)
	}
	if req.ExternalServiceID != nil && *req.ExternalServiceID > 0 {
		if vm.ExternalServiceID == nil || *vm.ExternalServiceID != *req.ExternalServiceID {
			return nil, "", true, fmt.Errorf("idempotency key belongs to a different external service: %w", sharederrors.ErrConflict)
		}
	}
	return vm, existingTask.ID, true, nil
}

func (s *VMService) vmFromTaskPayload(ctx context.Context, payloadJSON []byte) *models.VM {
	if s.vmRepo == nil {
		return nil
	}
	var payload struct {
	...
	if err := json.Unmarshal(payloadJSON, &payload); err != nil || payload.VMID == "" {
		return nil
	}
...
func (s *VMService) resolveExternalServiceCreateConflict(
	ctx context.Context,
	serviceID int,
	customerID string,
) (*models.VM, string, error) {
	existingVM, err := s.vmRepo.GetByExternalServiceID(ctx, serviceID)
	if err != nil {
		return nil, "", fmt.Errorf("creating VM record conflicted; lookup external service %d: %w", serviceID, err)
	}
	if existingVM.CustomerID != customerID {
		return nil, "", fmt.Errorf("external service %d belongs to another customer: %w", serviceID, sharederrors.ErrConflict)
	}
	s.logger.Info("VM create conflict resolved as idempotent external service retry",
		"vm_id", existingVM.ID, "external_service_id", serviceID, "customer_id", customerID)
	return nil, "", externalServiceWithoutTaskError(serviceID)
}

func externalServiceWithoutTaskError(serviceID int) error {
	return fmt.Errorf("external service %d already has a VM without a durable create task: %w", serviceID, sharederrors.ErrConflict)
}
...
func (s *VMService) getVMForDelete(ctx context.Context, vmID, customerID string, isAdmin bool) (*models.VM, error) {
	vm, err := s.vmRepo.GetByIDIncludingDeleted(ctx, vmID)
	if err != nil {
		if sharederrors.Is(err, sharederrors.ErrNotFound) {
...
func (s *VMService) GetVMDetail(ctx context.Context, vmID, customerID string, isAdmin bool) (*models.VMDetail, error) {
	...
	s.applyVMDisplayFields(&detail.VM, nil)
	...
	if s.ipamService != nil {
		...
		if err != nil {
			...
			s.applyVMDisplayFields(&detail.VM, ips)
		}
	}
...
func (s *VMService) enrichVMList(ctx context.Context, vms []models.VM) {
	for i := range vms {
		s.enrichVM(ctx, &vms[i])
	}
}

func (s *VMService) enrichVM(ctx context.Context, vm *models.VM) {
	s.applyVMDisplayFields(vm, nil)
	if s.ipamService == nil {
		return
	}
	...
	s.applyVMDisplayFields(vm, ips)
}

func (s *VMService) applyVMDisplayFields(vm *models.VM, ips []models.IPAddress) {
	if vm.Name == "" {
		vm.Name = vm.Hostname
	}
	if vm.IPv4 != "" {
	...
	for _, ip := range ips {
		if ip.IPVersion == 4 && ip.IsPrimary {
			vm.IPv4 = ip.Address
			return
		}
	}
...
func (p *DefaultTaskPublisher) PublishTask(ctx context.Context, taskType string, payload map[string]any) (string, error) {
	return p.PublishTaskWithIdempotencyKey(ctx, taskType, payload, "")
}

// PublishTaskWithIdempotencyKey creates a task record linked to an idempotency key.
func (p *DefaultTaskPublisher) PublishTaskWithIdempotencyKey(ctx context.Context, taskType string, payload map[string]any, idempotencyKey string) (string, error) {
