...
func (r *BillingTransactionRepository) CreditAccount(
	...
	if idempotencyKey != nil {
		existing, idErr := findMatchingIdempotencyKey(
			ctx, tx, customerID, *idempotencyKey,
			models.BillingTxTypeCredit, amount, nil, nil,
		)
		if idErr != nil || existing != nil {
			return existing, idErr
		}
	}
...
func findMatchingIdempotencyKey(
	ctx context.Context,
	tx pgx.Tx,
	customerID, key, txType string,
	amount int64,
	referenceType, referenceID *string,
) (*models.BillingTransaction, error) {
	existing, err := findByIdempotencyKey(ctx, tx, customerID, key)
	...
	if !billingTxMatches(existing, txType, amount, referenceType, referenceID) {
		return nil, fmt.Errorf("idempotency key %q conflicts with existing ledger entry: %w",
			key, sharederrors.ErrConflict)
	}
	return existing, nil
}

func billingTxMatches(
	...
	return tx.Type == txType &&
		tx.Amount == amount &&
		stringPtrEqual(tx.ReferenceType, referenceType) &&
		stringPtrEqual(tx.ReferenceID, referenceID)
}
