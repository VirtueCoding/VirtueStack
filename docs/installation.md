...
# Run migrations
make migrate-up
# Or with explicit URL (production: keep sslmode=require; only use sslmode=disable
# when connecting over a unix socket or loopback in local development):
migrate -path migrations -database "postgresql://virtuestack:${POSTGRES_PASSWORD}@localhost:5432/virtuestack?sslmode=require" up
```
...
- guest operation HMAC secret (32+ bytes; required for sensitive guest operations such as password resets)
...
GUEST_OP_HMAC_SECRET=replace-with-32-plus-byte-random-secret
```

Upgrade note: node-agent versions with guest-operation HMAC enforcement reject sensitive guest operations when `GUEST_OP_HMAC_SECRET` is unset; deployments that previously ran without it must set a shared 32+ byte secret before upgrading.
...
```bash
# Check current migration status (production: keep sslmode=require)
migrate -path migrations -database "postgresql://virtuestack:${POSTGRES_PASSWORD}@localhost:5432/virtuestack?sslmode=require" version
...
| REDIS_URL | No | - | Redis URL for distributed rate limiting (recommended when running multiple controller instances behind a load balancer; not currently enforced in production) |
...
**IMPORTANT:** When running multiple controller instances behind a load balancer, configure `REDIS_URL` so all instances share rate-limit counters. The controller does **not** currently refuse to start without `REDIS_URL`; if it is unset, each instance falls back to its own in-memory limiter, which can be bypassed by spreading requests across instances. For single-instance production deployments the in-memory limiter is sufficient.
