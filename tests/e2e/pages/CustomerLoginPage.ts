...
import { generateFreshTOTP } from '../utils/auth';
...
export class CustomerLoginPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.totpInput = page.locator('input[name="totp_code"], input#totp_code, .totp-input input');
    this.errorMessage = page.locator('[role="alert"], [data-testid="error"], [class*="bg-destructive"], [class*="text-destructive"]');
    this.forgotPasswordLink = page.locator('a:has-text("Forgot your password"), a:has-text("Forgot Password"), a:has-text("Reset password")');
