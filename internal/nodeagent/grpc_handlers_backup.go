...
func (h *grpcHandler) CreateLVMBackup(ctx context.Context, req *nodeagentpb.CreateLVMBackupRequest) (*nodeagentpb.CreateLVMBackupResponse, error) {
	...
	// NA-9: validate snapshot identifier and the *full* backup file path. The
	// previous implementation validated only filepath.Dir(backup_file_path),
	// which let a hostile file component escape the storage root. The
	// snapshot_name is also concatenated into `/dev/<vg>/<snap>` and passed to
	// ImageExists, so it must be a safe LVM identifier.
	if err := validateCreateLVMBackupArgs(req.GetSnapshotName(), req.GetBackupFilePath(), h.server.config.StoragePath); err != nil {
