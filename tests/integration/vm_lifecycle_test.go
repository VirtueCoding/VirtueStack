...
func forceSetVMStatus(ctx context.Context, suite *TestSuite, vmID, status string) error {
	_, err := suite.DBPool.Exec(ctx, "UPDATE vms SET status = $1, updated_at = NOW() WHERE id = $2", status, vmID)
	return err
}
...
func TestVMLifecycle(t *testing.T) {
	...
	t.Run("UpdateVMStatus", func(t *testing.T) {
		...
		err = suite.VMRepo.TransitionStatus(ctx, vmID, models.VMStatusProvisioning, models.VMStatusRunning)
	...
	t.Run("VMStartStop", func(t *testing.T) {
		...
		// Stop the VM
		err = suite.VMRepo.TransitionStatus(ctx, vmID, models.VMStatusRunning, models.VMStatusStopped)
		...
		// Start the VM again
		err = suite.VMRepo.TransitionStatus(ctx, vmID, models.VMStatusStopped, models.VMStatusRunning)
...
func TestVMStatusTransitions(t *testing.T) {
	...
	for _, tt := range validTransitions {
		t.Run(tt.from+"->"+tt.to, func(t *testing.T) {
			...
			// Set initial status when the VM must start from a non-default state.
			if tt.from != models.VMStatusProvisioning {
				err = forceSetVMStatus(ctx, suite, vmID, tt.from)
				require.NoError(t, err)
			}

			// Transition to new status
			err = suite.VMRepo.TransitionStatus(ctx, vmID, tt.from, tt.to)
	...
	t.Run("ConcurrentStatusUpdates", func(t *testing.T) {
		...
		for i := 0; i < 10; i++ {
			go func(idx int) {
				...
				done <- forceSetVMStatus(ctx, suite, vmID, status)
			}(i)
		}
