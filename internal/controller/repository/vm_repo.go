...
func cursorPageLimit(perPage int) uint64 {
	if perPage < 0 {
		return 0
	}
	return uint64(perPage) + 1
}
...
func (r *VMRepository) Create(ctx context.Context, vm *models.VM) error {
	...
	if err != nil {
		if isActiveExternalServiceDuplicate(err) {
			return fmt.Errorf("creating VM duplicate external service: %w", errors.Join(err, sharederrors.ErrConflict))
		}
...
func isActiveExternalServiceDuplicate(err error) bool {
	var pgErr *pgconn.PgError
	return errors.As(err, &pgErr) &&
		pgErr.Code == "23505" &&
		pgErr.ConstraintName == "idx_vms_external_service_id_active_unique"
}
...
func (r *VMRepository) GetByIDIncludingDeleted(ctx context.Context, id string) (*models.VM, error) {
	const q = `SELECT ` + vmSelectCols + ` FROM vms WHERE id = $1`
	vm, err := ScanRow(ctx, r.db, q, []any{id}, scanVM)
	if err != nil {
		return nil, fmt.Errorf("getting VM %s including deleted: %w", id, err)
	}
	return &vm, nil
}
...
func (r *VMRepository) List(ctx context.Context, filter models.VMListFilter) ([]models.VM, bool, string, error) {
	...
	whereBuilder := baseBuilder.Select(vmSelectCols).From("vms").Where("deleted_at IS NULL")
	...
	listBuilder = listBuilder.
		OrderBy("id DESC").
		Limit(cursorPageLimit(filter.PerPage))
...
// RestoreSoftDelete clears a soft delete marker after a failed delete dispatch.
func (r *VMRepository) RestoreSoftDelete(ctx context.Context, id, previousStatus string) error {
	const q = `UPDATE vms SET deleted_at = NULL, status = $1, updated_at = NOW() WHERE id = $2 AND deleted_at IS NOT NULL`
	tag, err := r.db.Exec(ctx, q, previousStatus, id)
	if err != nil {
		return fmt.Errorf("restoring soft-deleted VM %s: %w", id, err)
	}
	if tag.RowsAffected() == 0 {
		return fmt.Errorf("restoring soft-deleted VM %s: %w", id, ErrNoRowsAffected)
	}
	return nil
}
