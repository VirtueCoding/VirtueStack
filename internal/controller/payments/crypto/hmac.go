...
func verifyNOWPaymentsSignature(secret, signature string, body []byte) error {
	...
	// Use json.Decoder with UseNumber() so that large integers (>2^53) and
	// fixed-decimal amounts survive the decode/re-marshal round-trip
	// verbatim. Decoding into map[string]any without UseNumber() would
	// coerce JSON numbers to float64 and silently mutate values such as
	// 9007199254740993, breaking the canonical body and the HMAC.
