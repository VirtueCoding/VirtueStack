...
func RegisterAdminRoutes(router *gin.RouterGroup, handler *AdminHandler, inAppNotifHandler *AdminInAppNotificationsHandler) {
	...
	// Protected auth read endpoints require valid JWT but not CSRF.
	...
	// Protected auth mutation endpoints require valid JWT and CSRF.
	protectedAuthMutations := admin.Group("/auth")
	protectedAuthMutations.Use(middleware.JWTAuth(handler.authConfig))
	protectedAuthMutations.Use(middleware.RequireRole("admin", "super_admin"))
	protectedAuthMutations.Use(middleware.CSRF(middleware.DefaultCSRFConfig()))
	{
		protectedAuthMutations.POST("/reauth", handler.Reauth)
		protectedAuthMutations.POST("/logout", handler.Logout)
	}
