...
// TestCreateImageValidationRejectsBadIdentifier covers NA-7: CreateImage must
// validate the imageName using validateLVIdentifier before invoking lvcreate.
// We exercise only the rejection paths so the test does not require a real
// LVM volume group; valid names are not exercised here because the success
// path would shell out to lvcreate.
func TestCreateImageValidationRejectsBadIdentifier(t *testing.T) {
	mgr, err := NewLVMManager("vgvs", "thinpool", slog.Default())
	if err != nil {
		t.Fatalf("NewLVMManager: %v", err)
	}
	...
	tests := []struct {
		...
		{"path traversal", ".."},
		{"option flag short", "-h"},
		{"option flag long", "--help"},
		{"path separator", "a/b"},
		{"semicolon shell injection", "vm;rm -rf /"},
		{"comma", "vm,foo=bar"},
		{"space", "vm name"},
		{"newline", "vm\nfoo"},
	}
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			err := mgr.CreateImage(context.Background(), tc.imageName, 1)
			if err == nil {
				t.Fatalf("CreateImage(%q) returned nil error, want validation rejection", tc.imageName)
			}
			if !strings.Contains(err.Error(), "invalid LVM identifier") &&
				!strings.Contains(err.Error(), "cannot be empty") {
				t.Fatalf("CreateImage(%q) error %q does not look like an identifier-validation failure",
					tc.imageName, err.Error())
			}
		})
	}
}
