...
// NewNodeService constructs a NodeService from explicit dependencies.
...
// NewNodeServiceWithDefaults constructs a NodeService with the minimal dependencies used by older call sites.
...
// UndrainNode returns a draining node to online placement status.
...
// GetFailoverStats returns the current circuit-breaker state for a node's failover attempts.
func (s *NodeService) GetFailoverStats(nodeID string) CircuitBreakerStats {
	return s.circuitBreaker.GetFailoverStats(nodeID)
}

// ResetFailoverCircuitBreaker clears failover circuit-breaker state for the given node.
...
// UpdateNode validates and persists mutable node attributes.
