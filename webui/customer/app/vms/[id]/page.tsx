...
export default function VMDetailPage() {
  ...
  useEffect(() => {
    if (activeTab !== "backups" || !vmId) {
      return;
    }
    fetchBackups();
    fetchSnapshots();
  }, [activeTab, fetchBackups, fetchSnapshots, vmId]);
  ...
  const displayName = vm.name?.trim() || vm.hostname;
  const primaryIPv4 =
    vm.ipv4?.trim() ||
    vm.ip_addresses?.find((ip) => ip.ip_version === 4 && ip.is_primary)?.address ||
    "Not assigned";
  const showHostname = vm.hostname !== displayName;
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <p className="mt-1 font-mono text-lg">{primaryIPv4}</p>
            </div>
            <div>
          ...
          <Card>
            ...
            <CardContent>
              {vm?.status !== "running" ? (
                ...
                <SerialConsoleWithToken vmId={vmId} vmName={displayName} />
              )}
            </CardContent>
          </Card>
        </TabsContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {primaryIPv4 !== "Not assigned" && (
                  <div className="rounded-lg border p-4">
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      <Network className="h-4 w-4" />
                      <span>IPv4 Address</span>
                    </div>
                    <p className="mt-1 font-mono text-lg font-semibold">{primaryIPv4}</p>
                  </div>
                )}
