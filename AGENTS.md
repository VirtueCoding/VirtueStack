...
```
POST   /api/v1/webhooks/stripe   # Stripe webhook (verified via stripe-go)
POST   /api/v1/webhooks/paypal   # PayPal webhook (verified via PayPal API)
POST   /api/v1/webhooks/crypto   # BTCPay/NOWPayments webhook (HMAC signature)
```

These endpoints are mounted on the `/api/v1` router group but skip the JWT/API-key auth middleware. Each handler verifies the webhook signature using the respective provider's SDK before processing.
...
Listening port: 3000 (both dev and production; see `ADMIN_PORT` in `.env.example`).
...
Listening port: 3001 (both dev and production; see `CUSTOMER_PORT` in `.env.example`).
...
| `BILLING_WHMCS_ENABLED` | No | Enable WHMCS billing adapter (`true`/`false`) |
| `BILLING_WHMCS_PRIMARY` | No | Mark WHMCS as the primary billing provider |
| `BILLING_NATIVE_ENABLED` | No | Enable native (built-in) billing adapter |
| `BILLING_NATIVE_PRIMARY` | No | Mark native billing as primary |
| `BILLING_BLESTA_ENABLED` | No | Enable Blesta billing adapter |
| `BILLING_BLESTA_PRIMARY` | No | Mark Blesta as primary |
| `BILLING_GRACE_PERIOD_HOURS` | No | Grace period before suspending unpaid VMs (default 12) |
| `BILLING_WARNING_INTERVAL_HOURS` | No | Hours between low-balance warnings (default 24) |
| `BILLING_NATIVE_AUTO_DELETE_DAYS` | No | Days after suspension before auto-delete (0 disables) |
| `BILLING_INVOICE_STORAGE_PATH` | No | Filesystem path for generated invoice PDFs |
| `BILLING_INVOICE_PREFIX` | No | Invoice number prefix (e.g. `INV-`) |
...
| `PAYPAL_MODE` | No | `sandbox` or `live` (default `sandbox`) |
| `PAYPAL_WEBHOOK_ID` | No | PayPal webhook ID for verification |
| `PAYPAL_RETURN_URL` | No | Return URL after PayPal checkout |
| `PAYPAL_CANCEL_URL` | No | Cancel URL after PayPal checkout |
| `CRYPTO_PROVIDER` | No | Crypto gateway: `btcpay`, `nowpayments`, or `disabled` |
| `BTCPAY_SERVER_URL` | No | BTCPay Server base URL |
| `BTCPAY_API_KEY` | No | BTCPay Server Greenfield API key |
...
| `NOWPAYMENTS_CALLBACK_URL` | No | NOWPayments IPN callback URL |
| `OAUTH_GOOGLE_ENABLED` | No | Enable Google OAuth login |
| `OAUTH_GOOGLE_CLIENT_ID` | No | Google OAuth client ID |
| `OAUTH_GOOGLE_CLIENT_SECRET` | No | Google OAuth client secret |
| `OAUTH_GITHUB_ENABLED` | No | Enable GitHub OAuth login |
...
| `GUEST_OP_HMAC_SECRET` | Yes | 32+ byte secret for sensitive guest-operation HMAC tokens |
...
```bash
pnpm install --frozen-lockfile --ignore-scripts      # NOT npm; Playwright browsers installed separately
